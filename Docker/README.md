...
The above will build off the most recent commit to the master branch by default. If you would like to target a specific branch/tag, you may use a build argument. For example, if you wished to generate a docker image based off of the v1.0.1-1.4.3 tag, you could do the following:

```bash
docker build -t boscore/bos:v1.0.1-1.4.3 --build-arg branch=v1.0.1-1.4.3 .
```
